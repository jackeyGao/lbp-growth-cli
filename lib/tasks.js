 ...
 * Task management
 */
const { TASKS_URL, BASE_URL, BLOCKLIST_URL, BATCH_CONFIRM_URL, UNSEND_URL } = require('./config');
const { httpGet, httpPost, httpPut, httpDelete, httpDownload } = require('./http');
const fs = require('fs');
...
async function getTask(ak, taskId) {
  ...
  return {
    ...
    stage: 'get',
    task_id: taskId,
    message: `获取任务详情失败: HTTP ${status}`,
    hint,
    body: body || null,
  };
}

async function listTaskBatches(ak, taskId, { page = 1, pageSize = 100 } = {}) {
  const qs = new URLSearchParams({ page: String(page), pageSize: String(pageSize) });
  const url = `${TASKS_URL}/${taskId}/batches?${qs}`;
  const { status, body } = await httpGet(url, ak);
  ...
  return {
    ...
    stage: 'batches',
    task_id: taskId,
    message: `获取任务批次列表失败: HTTP ${status}`,
    hint,
    body: body || null,
  };
}
...
async function unsendTask(ak, taskId, appIds = []) {
  const payload = { task_id: taskId };
  if (appIds && appIds.length > 0) {
    payload.app_ids = appIds;
  }

  const { status, body } = await httpPost(UNSEND_URL, ak, payload);

  if (status >= 200 && status < 300) {
    // 成功时 body 为上游原始响应: { code, msg, data }
    ...
    return {
      ...
      ...(isSuccess && body.data ? {
        ...
        success_count: body.data.success_count,
        skipped_count: body.data.skipped_count,
      } : {}),
      ...(isSuccess ? {} : { message: body.msg || '上游返回非成功状态码' }),
    };
  }
  ...
  if (status === 400) {
    ...
    hint = `此任务尚未提交撤回请求，无法撤回。请先访问 ${BASE_URL}/recall/${taskId} 提交撤回请求，提交后才能执行撤回操作。`;
  } else if (status === 404) {
    hint = '任务 id 不存在，请用 `lbp-growth tasks list` 确认 id 是否正确';
  } else if (status === 500) {
    hint = errorMessage || '上游接口异常，请稍后重试';
  }
  ...
  return {
    ...
    stage: 'unsend',
    task_id: taskId,
    message: errorMessage || `撤回失败: HTTP ${status}`,
    hint,
    status,
  };
}
