  ...
  if (!bearerToken) {
    try {
      ...
      if (status >= 200 && status < 300 && body.valid === true) {
        return { valid: true, info: `valid (userId=${body.userId || 'unknown'}, roles=${JSON.stringify(body.roles || [])})`, userId: body.userId, roles: body.roles || [] };
      }
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode >= 200 && res.statusCode < 300 && body.valid === true) {
            resolve({ valid: true, info: `valid (userId=${body.userId || 'unknown'}, roles=${JSON.stringify(body.roles || [])})`, userId: body.userId, roles: body.roles || [] });
          } else {
...
async function checkAuth() {
  ...
  if (creds.apiKey) {
    const { valid, info, userId, roles } = await verifyAk(creds.apiKey, bearerToken);
    results.api_key = {
      status: valid ? 'valid' : 'invalid',
