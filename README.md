...
| `auth save <API_KEY> <BEARER_TOKEN>` | 保存并校验 API Key 和 Bearer Token |
| `auth show` | 显示当前凭据（脱敏） |
...
| `--content` | card时 | 卡片内容，必须是有效 JSON 字符串 |
| `--release-note-ids` | releaseNote时 | Release Note ID 列表，逗号分隔 |
...
| `--schedule-time` | scheduled时 | 定时时间（ISO 格式） |
| `--confirm` | 否 | 确认执行真实推送 |
